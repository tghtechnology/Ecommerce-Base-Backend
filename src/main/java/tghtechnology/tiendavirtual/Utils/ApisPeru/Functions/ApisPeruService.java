...
public class ApisPeruService {
	...
	public ApisPeruResponse enviarBoleta(Boleta boleta) throws IOException, ApisPeruResponseException {
		...
		headers.setBearerAuth(settings.getString("apisperu.token"));
		try {
			HttpEntity<Boleta> entity = new HttpEntity<>(boleta, headers);
			
			String result = rt.postForObject(settings.getString("apisperu.url") + "/invoice/send", entity ,String.class);
	...
	public byte[] enviarBoletaPdf(Boleta bol) throws ApisPeruResponseException {
		...
		byte[] result = rt.postForObject(settings.getString("apisperu.url") + "/invoice/pdf", entity ,byte[].class);
		return result;
		//return result;
		} catch(HttpStatusCodeException ex) {
			HttpStatusCode status = ex.getStatusCode();
